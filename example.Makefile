...
install:
	...
	@cp example.entrypoint.prod.sh entrypoint.prod.sh || echo "$(COL_YELLOW)Something went wrong. Please, copy the example.entrypoint.prod.sh file to entrypoint.prod.sh manually.$(COL_WHITE)"
	@echo "$(COL_GREEN)"
...
build:
	...
	@docker build -t $(IMG_NAME):$(IMG_TAG) . --target dev && \
...
build-prod:
	@echo "Building the project..."
	@docker build -t $(IMG_NAME):$(IMG_TAG) . --target prod && \
...
start:
	...
	@chown -R $(shell id -u):$(shell id -g) ./src || echo "$(COL_YELLOW)\nSomething went wrong. Please, change the permissions of the src folder manually.$(COL_WHITE)"
	@chmod -R ug+rwx ./src || echo "$(COL_YELLOW)\nSomething went wrong. Please, change the permissions of the src folder manually.$(COL_WHITE)"
	@echo "$(COL_WHITE)Starting the project...$(COL_DEF)"
	@docker run -it -p $(PORT_OUT):$(PORT_IN) -v ./src:/app -u $(shell id -u):$(shell id -g) --name $(CNT_NAME) $(IMG_NAME):$(IMG_TAG) || echo $(DEF_ERR_MSG)

# This target will be executed if you run `make start-prod`
start-prod:
	@echo "$(COL_WHITE)Starting the project...$(COL_DEF)"
	@docker run -d -p $(PORT_OUT):$(PORT_IN) --name $(CNT_NAME) $(IMG_NAME):$(IMG_TAG) || echo $(DEF_ERR_MSG)
...
logs:
	@docker logs $(IMG_NAME) -f || echo $(DEF_ERR_MSG)
