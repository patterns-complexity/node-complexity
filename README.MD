...
Either place your existing Node.js application code in the `src` directory, or start developing it from scratch.
...
## Production use
Do **NOT** trust any production images or containers that are built from this project. They are only meant for testing and simulating a production environment. Always build your own images and containers for production.
...
Refer to the [example.env](example.env) to see the default values.
...
- [example.entrypoint.dev.sh](example.entrypoint.dev.sh)
- [example.entrypoint.prod.sh](example.entrypoint.prod.sh)
